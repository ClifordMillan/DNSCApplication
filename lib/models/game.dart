...
import 'package:flutterproject/utils/icons.dart';
import 'package:flutterproject/models/card_item.dart';
...
class Game {
  ...
  void generateCards() {
    ...
    for (int i = 0; i < (gridSize * gridSize / 2); i++) {
      final cardValue = i + 1;
      final String icon = icons.elementAt(i);
  ...
  void generateIcons() {
    ...
    for (int j = 0; j < (gridSize * gridSize / 2); j++) {
      final String icon = _getRandomCardIcon();
      icons.add(icon);
  ...
  List<CardItem> _createCardItems(
       String  icon, Color cardColor, int cardValue) {
  ...
  String  _getRandomCardIcon() {
    final Random random = Random();
   String  icon;
    do {
      icon = cardIcons[random.nextInt(cardIcons.length)] as  String ;
